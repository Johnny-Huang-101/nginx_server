...
from lims.models import Cabinets, Locations, CabinetTypes, Statuses, Specimens, Cases, Containers, SpecimenTypes
from lims.cabinets.forms import Add, Edit, Approve, Update
from lims.cabinets.functions import get_form_choices
...
def view_list():
    ...
    kwargs['types'] = [item.name for item in CabinetTypes.query]
    ...
    if query_type == 'type':
        if query:
            items = table.query.join(CabinetTypes).filter(CabinetTypes.name == query)
