...
from lims.models import Agencies, Divisions, Personnel, LitPacketAdmins, Assays, LitPacketAdminTemplates
from lims.lit_packet_admins.forms import Add, Edit, Approve, Update
from lims.forms import Attach, Import
from lims.view_templates.views import *
from lims.lit_packet_admins.functions import *

# Set item global variables
item_type = 'Lit Packet Admin'
item_name = 'Lit Packet Admins'
table = LitPacketAdmins
table_name = 'lit_packet_admins'
...
def add():
    ...
    form = Add()
    # form.lit_admin_template_id.choices = [(str(t.id), t.name) for t in LitPacketAdminTemplates.query.all()]
    form.lit_admin_template_id.choices = [(t.id, {t.name}) for t in LitPacketAdminTemplates.query.all()]
    if form.is_submitted():
        # form.lit_admin_template_id = form.lit_admin_template_id.data
        print(f'DATA HERE {form.lit_admin_template_id.data}')
