...
from lims.case_types.forms import Add, Edit, Approve, Update
from lims.case_types.functions import get_form_choices, get_orders
from lims.forms import Attach, Import
# Application Imports
from lims.models import CaseTypes, ReportTemplates
from lims.view_templates.views import *

# Item Global Variables
item_type = 'Case Type'
item_name = 'Case Types'
table = CaseTypes
...
def add():
    ...
    kwargs['accession_levels'], kwargs['batch_levels'] = get_orders()
    if request.method == 'POST':
        kwargs['current_case_number'] = form.case_number_start.data
...
@blueprint.route(f'/{table_name}/<int:item_id>/edit_<event>', methods=['GET', 'POST'])
@login_required
def edit(item_id):
    kwargs = default_kwargs.copy()
...
@blueprint.route(f'/{table_name}/<int:item_id>/approve_<event>', methods=['GET', 'POST'])
@login_required
def approve(item_id):
    kwargs = default_kwargs.copy()
...
@blueprint.route(f'/{table_name}/get_templates/')
@login_required
def get_templates():

    discipline = request.args.get('discipline', type=int)

    templates = ReportTemplates.query.filter_by(discipline=discipline, db_status='Active')
    choices = []

    if discipline:
        if templates.count():
            choices.append({'id': 0, 'name': 'Please select a report template'})
            ...
            choices.append({'id': 0, 'name': 'This discipline has no report template'})
    else:
