...
from lims.united_states.forms import Add, Edit, Approve, Update

# Set item global variables
item_type = 'State'  # singular
...
redirect_to = 'list'  # Where to redirect after form submission
...
def add():
    ...
    form = Add()
    _add = add_item(form, table, item_type, item_name, table_name, requires_approval, name, admin_only=True, **kwargs)
    return _add
...
def approve_remove(item_id):
    _approve_remove = approve_remove_item(item_id, table, table_name, item_name, name, admin_only=True)
...
def reject_remove(item_id):
    _reject_remove = reject_remove_item(item_id, table, table_name, item_name, name, admin_only=True)
...
def delete(item_id):
    ...
    _delete_item = delete_item(form, item_id, table, table_name, item_name, name, admin_only=True)
...
def attach(item_id):
    ...
    _attach = attach_items(form, item_id, table, item_name, table_name, name, admin_only=True)
...
def view(item_id):
    ...
    alias = f"{getattr(item, name)} | {getattr(item, 'abbreviation')}"

    _view = view_item(item, alias, item_name, table_name, view_only=True)
    return _view
