...
from lims.models import SpecimenCollectionContainers, SpecimenCollectionContainerTypes
from lims.specimen_collection_containers.forms import Add, Edit, Approve, Update
from lims.specimen_collection_containers.functions import get_form_choices
from lims.forms import Attach, Import
from lims.view_templates.views import *

# Item Global Variables
item_type = 'Collection Vessel'
item_name = 'Collection Vessels'
table = SpecimenCollectionContainers
table_name = "specimen_collection_containers"
name = 'display_name'  # This selects what property is displayed in the flash messages
...
def add():
    ...
    if request.method == 'POST':
        if form.is_submitted() and form.validate():
            ...
            vessel_type = SpecimenCollectionContainerTypes.query.get(form.type_id.data)
            kwargs['display_name'] = f"{vessel_type.name} ({form.name.data})" if form.name.data is not None else f"{vessel_type.name}"
...
def edit(item_id):
    ...
    if request.method == 'POST':
        if form.is_submitted() and form.validate():
            vessel_type = SpecimenCollectionContainerTypes.query.get(form.type_id.data)
            kwargs.update({'display_name': f"{vessel_type.name} ({form.name.data})" if form.name.data is not None else f"{vessel_type.name}"})
