...
from lims.models import Salts
from lims.forms import Attach, Import
from lims.view_templates.views import *

from lims.salts.forms import Add, Edit, Approve, Update
...
def update(item_id):
    kwargs = default_kwargs.copy()
    form = Update()
    _update = update_item(form, item_id, table, item_type, item_name, table_name, requires_approval,
                          name, admin_only=True, **kwargs)
...
@login_required
def view_list():
    _view_list = view_items(table, item_name, item_type, table_name, view_only=True)
