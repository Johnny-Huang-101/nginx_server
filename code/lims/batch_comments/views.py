...
from lims.models import BatchComments, Batches
from lims.batch_comments.forms import Add, Edit, Approve, Update
from lims.batch_comments.functions import get_form_choices
from lims.forms import Attach, Import
from lims.view_templates.views import *


# Set item global variables
item_type = 'Batch Comment'  # singular
item_name = 'Batch Comments'
table = BatchComments
table_name = 'batch_comments'
...
@blueprint.route(f'/{table_name}/<int:batch_id>/add', methods=['GET', 'POST'])
@login_required
def add_from_batch(batch_id):
    # Used for adding a comment directly from the batch
    kwargs = default_kwargs.copy()
    form = get_form_choices(Add())

    batch = Batches.query.get(batch_id)
    form.batch_id.data = batch.id
