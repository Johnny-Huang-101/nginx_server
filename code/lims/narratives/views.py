...
from lims.narratives.forms import Add, Edit, Approve, Update
...
def update(item_id):
    ...
    kwargs['narrative_text'] = item.narrative
    # If the narrative is updated redirect to the case view
    kwargs['redirect'] = url_for('cases.view', item_id=item.case.id)
    alias = f"{item.case.case_number} - {item.narrative_type}"
    if request.method == 'POST':
        # if the <mark> tag is present in the narrative i.e., the narrative was highlighted
        # set checked to 'Y' or None
        if "<mark>" in form.narrative.data:
            kwargs['checked'] = 'Y'
        else:
            kwargs['checked'] = None
...
def view(item_id):
    ...
    _view = view_item(item, alias, item_name, table_name, default_header=False)
    return _view
