...
from wtforms import StringField, TextAreaField, SubmitField, IntegerField, SelectField, DateField, HiddenField
from wtforms.validators import DataRequired, Optional


class Base(FlaskForm):
    # TODO remove 'name' HiddenField here and in views.py when removed from models
    name = HiddenField('Name', validators=[DataRequired()])

    equipment_id = StringField('Equipment ID', validators=[DataRequired()])
    serial_number = StringField('Serial Number', validators=[DataRequired()])
    date_acquired = DateField("Acquired Date", validators=[Optional()])
    status_id = SelectField('Status', coerce=int, validators=[DataRequired()])
    last_serviced_date = DateField('Last Service Date', validators=[Optional()])
