from lims.general_labware.functions import get_form_choices
from lims.locations.functions import set_location, models_iter, location_dict, get_location_display
from lims.models import GeneralLabware, Locations, GeneralLabwareTypes, Statuses
from lims.general_labware.forms import Add, Edit, Approve, Update
from lims.forms import Attach, Import
from lims.view_templates.views import *

# Set item global variables
item_type = 'General Labware'
item_name = 'General Labware'
table = GeneralLabware
table_name = 'general_labware'
...
def add():
    ...
    if form.is_submitted():
        form.name.data = form.equipment_id.data
...
def view_list():
    ...
    kwargs['types'] = [item.name for item in GeneralLabwareTypes.query]
    ...
    if query_type == 'type':
        if query:
            items = table.query.join(GeneralLabwareTypes).filter(GeneralLabwareTypes.name == query)
    ...
    # Filter based on calbration due date
    if query_type == 'service-date':
        if query == 'Overdue':
            items = table.query.join(Statuses).filter(
                sa.and_(
    ...
    _view_list = view_items(table, item_name, item_type, table_name,
                            items=items, order_by=['equipment_ID'], **kwargs)
