...
from lims.models import FumeHoods, Locations, HoodTypes, Statuses, Specimens, Cases, Containers, SpecimenTypes
from lims.fume_hoods.forms import Add, Edit, Approve, Update
from lims.fume_hoods.functions import get_form_choices
...
item_type = 'Hood'  # singular
item_name = 'Hoods'
table = FumeHoods
table_name = 'fume_hoods'
...
def view_list():
    ...
    kwargs['types'] = [item.name for item in HoodTypes.query]
    ...
    if query_type == 'type':
        if query:
            items = table.query.join(HoodTypes).filter(HoodTypes.name == query)
    ...
    _view_list = view_items(table, item_name, item_type, table_name,
                            items=items, order_by=['equipment_ID'],
