...
from lims.drug_classes.forms import Add, Edit, Approve, Update

# Set item global variables
item_type = 'Drug Class'
item_name = 'Drug Classes'
table = DrugClasses
...
def view_list():
    ...
    # Recalculate n_compounds and components
    for drug_class in table.query:
        drug_class.n_compounds = Compounds.query.filter_by(drug_class_id=drug_class.id).count()
        drug_class.n_components = Components.query.filter_by(drug_class_id=drug_class.id).count()

    _view_list = view_items(table, item_name, item_type, table_name, order_by=['name'], user_perms=user_perms)
...
def view(item_id):
    ...
    # Recalculate n_compounds and components
    item.n_compounds = Compounds.query.filter_by(drug_class_id=item_id).count()
    item.n_components = Components.query.filter_by(drug_class_id=item_id).count()

    # Get compounds and components to display
    compounds = Compounds.query.filter_by(drug_class_id=item_id).order_by(Compounds.name)
    components = Components.query.filter_by(drug_class_id=item_id).order_by(Components.rank)

    _view = view_item(item, alias, item_name, table_name, compounds=compounds, components=components)
    return _view
