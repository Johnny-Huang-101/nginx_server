from lims import db
from lims.models import Agencies, Divisions, Statuses, InstrumentTypes
...
def get_form_choices(form):
    ...
    form.instrument_type_id.choices = [(item.id, item.name) for item in InstrumentTypes.query.all()]
    form.instrument_type_id.choices.insert(0, (0, 'Please select a type'))
