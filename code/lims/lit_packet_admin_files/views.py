...
from lims.lit_packet_admin_files.forms import *
from lims.forms import Attach, Import
from lims.view_templates.views import *

# Set item global variables
item_type = 'Lit Packet Admin File'
item_name = 'Lit Packet Admin Files'
table = LitPacketAdminFiles
table_name = 'lit_packet_admin_files'
...
def add():
    ...
    assay_id = request.args.get('template_id')
    form.lit_packet_admin_id.choices = [(t.id, t.name) for t in LitPacketAdminAssays.query.filter_by(id=assay_id)]
    print(assay_id)
    # form.use_file.choices = file_choices
    redact_type = [('Redact', 'Redact'), ('Delete Pages', 'Delete Pages'), ('None', 'None')]
    form.redact_type.choices = redact_type
    _add = add_item(form, table, item_type, item_name, table_name, requires_approval, 'file_name', **kwargs)
    if form.is_submitted():
        return redirect(f'/lit_packet_admin_assays/{form.lit_packet_admin_id.data}')
    return _add
...
def update(item_id):
    ...
    form.redact_type.choices = redact_type
    if form.is_submitted():
        _update = update_item(form, item_id, table, item_type, item_name, table_name, requires_approval, 'file_name',
                              **kwargs)
        return redirect(f'/lit_packet_admin_assays/{form.lit_packet_admin_id.data}')

    _update = update_item(form, item_id, table, item_type, item_name, table_name, requires_approval, 'file_name', **kwargs)
...
def delete(item_id):
    ...
    item = LitPacketAdminFiles.query.get(item_id)

    assay = LitPacketAdminAssays.query.get(item.lit_packet_admin_id)
    _delete_item = delete_item(form, item_id, table, table_name, item_name, 'file_name')
    return redirect(f'/lit_packet_admin_assays/{assay.id}')
    # return _delete_item
