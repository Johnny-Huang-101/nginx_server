...
from lims.models import QRReference
from lims.qr_reference.forms import Add, Edit, Approve, Update
from lims.forms import Attach, Import
from lims.view_templates.views import *
import qrcode

# Set item global variables
item_type = 'QR Reference'
item_name = 'QR Reference'
table = QRReference
table_name = 'qr_reference'
name = 'text'  # This selects what property is displayed in the flash messages
...
def add():
    ...
    if form.is_submitted() and form.validate():
        # Get ID of new entry for QR Code generation
        if len(QRReference.query.all()) == 0:
            current_id = 0
        else:
            current_id = QRReference.query.all()[-1].id

        # Set qr_path
        form.qr_path.data = os.path.join(current_app.root_path, 'static', 'filesystem', 'qr_reference',
                                         f'{str(current_id + 1)}.png')
        # Make new qr code
        new_qr = qrcode.make(f'qr_reference: {str(current_id + 1)}')
        # Save qr code
        new_qr.save(form.qr_path.data)
