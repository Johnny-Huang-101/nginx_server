...
from lims.personnel.forms import Add, Edit, Approve, Update
from lims.personnel.functions import get_form_choices, process_form, check_duplicate_personnel
...
def add():
    ...
    if request.method == 'POST':
        ...
        check_duplicate_personnel(form)
        if current_user.permissions not in ['Admin', 'Owner']:
            requires_approval = True
        
    _add = add_item(form, table, item_type, item_name, table_name,
...
def edit(item_id):
    ...
    form = get_form_choices(Edit(), agency_id=item.agency.id)
    if request.method == 'POST':
        kwargs.update(process_form(form))
...
def approve(item_id):
    kwargs = default_kwargs.copy()
    item = table.query.get(item_id)
    form = get_form_choices(Approve(), agency_id=item.agency.id)
    if request.method == 'POST':
        kwargs.update(process_form(form))
...
def update(item_id):
    ...
    form = get_form_choices(Update(), agency_id=item.agency.id)
    if request.method == 'POST':
        kwargs.update(process_form(form))
...
def import_file():
    form = Import()
    _import = import_items(form, table, table_name, item_name, dtype={'id_number': str})
...
def view_list():
    ...
    _view_list = view_items(table, item_name, item_type, table_name,
                            length=500, order_by=['full_name'], **kwargs)
...
def get_divisions():
    ...
    divisions = Divisions.query.filter_by(agency_id=agency_id, db_status='Active').all()
    choices = []
    if agency_id:
        if len(divisions):
            choices = [{'id': division.id, 'name': division.name} for division in divisions]
            choices.insert(0, {'id': 0, 'name': '---'})
        else:
