...
from lims.models import Benches, Locations, BenchTypes, Statuses, Specimens, Cases, Containers, SpecimenTypes
from lims.benches.forms import Add, Edit, Approve, Update
from lims.benches.functions import get_form_choices
...
def view_list():
    ...
    kwargs['types'] = [item.name for item in BenchTypes.query]
    ...
    if query_type == 'type':
        if query:
            items = table.query.join(BenchTypes).filter(BenchTypes.name == query)
