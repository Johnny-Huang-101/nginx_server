from lims import db
from lims.models import Statuses, BenchTypes
...
def get_form_choices(form):
    ...
    form.bench_type.choices = [(item.id, item.name) for item in BenchTypes.query.all()]
    form.bench_type.choices.insert(0, (0, 'Please select a type'))
