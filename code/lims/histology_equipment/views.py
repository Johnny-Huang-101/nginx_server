from lims.histology_equipment.functions import get_form_choices
from lims.locations.functions import set_location, models_iter, location_dict, get_location_display
from lims.models import HistologyEquipment, Locations, HistologyEquipmentTypes, Statuses
from lims.histology_equipment.forms import Add, Edit, Approve, Update
from lims.forms import Attach, Import
from lims.view_templates.views import *

# Set item global variables
item_type = 'Histology Equipment'
item_name = 'Histology Equipment'
table = HistologyEquipment
table_name = 'histology_equipment'
...
def view_list():
    ...
    kwargs['types'] = [item.name for item in HistologyEquipmentTypes.query]
    ...
    # Filter based on histo equipment type
    if query_type == 'type':
        if query:
            items = table.query.join(HistologyEquipmentTypes).filter(HistologyEquipmentTypes.name == query)
