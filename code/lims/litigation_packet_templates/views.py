...
from lims.models import Zipcodes, LitigationPacketTemplates
from lims.litigation_packet_templates.forms import Add, Edit, Approve, Update
from lims.forms import Attach, Import
from lims.view_templates.views import *

# Set item variables
item_type = 'Litigation Packet Template'
item_name = 'Litigation Packet Templates'
table = LitigationPacketTemplates
table_name = 'litigation_packet_templates'
...
# Create blueprint
blueprint = Blueprint('litigation_packet_templates', __name__)
...
def add():
    ...
    if form.is_submitted() and form.validate():
            file = form.file.data
            print(file)
            path = os.path.join(current_app.root_path, 'static\litigation_packet_templates', f"{form.name.data}.docx")
            print(path)
            file.save(path)
            form.path.data = path
...
def delete(item_id):
    ...
    # doc_delete = LitigationPacketTemplates.query.get(item_id).path
    # print(doc_delete)
    # litpacktemp = LitigationPacketTemplates.query.get(item_id)
    # doc_delete = litpacktemp.path
    # print(f"Attempting to delete: {doc_delete}")
    #
    # # Normalize the path to handle slashes correctly
    # doc_delete = os.path.normpath(doc_delete)
    # print(f"Normalized path: {doc_delete}")
    #
    # # Attempt to delete the file
    # try:
    #     if os.path.exists(doc_delete):
    #         os.remove(doc_delete)
    #         print(f"File {doc_delete} deleted successfully.")
    #     else:
    #         print(f"File {doc_delete} does not exist.")
    # except OSError as error:
    #     print(f"Error: {error}")
...
def view(item_id):
    ...
    alias = f"{getattr(item, name)} | {getattr(item, 'neighborhood')}"
