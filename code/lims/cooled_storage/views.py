...
from lims.models import CooledStorage, Locations, CooledStorageTypes, Statuses, Specimens, Cases, Containers, SpecimenTypes
from lims.cooled_storage.forms import Add, Edit, Approve, Update
from lims.cooled_storage.functions import get_form_choices
...
item_type = 'Cooled Storage'  # singular
item_name = 'Cooled Storage'
table = CooledStorage
table_name = 'cooled_storage'
...
def view_list():
    ...
    kwargs['types'] = [item.name for item in CooledStorageTypes.query]
    ...
    filter_message = None
    # Filter based on cooled storage type
    if query_type == 'type':
        if query:
            items = table.query.join(CooledStorageTypes).filter(CooledStorageTypes.name == query)
   ...
   # Service Date within 30 days
    ...
    danger_alerts = [
        (url_for(f'{table_name}.view_list', query='service-past-due'), service_past_due, Markup('with <b>service dates past due</b>')),
    ]


    _view_list = view_items(table, item_name, item_type, table_name,
                            items=items, filter_message=filter_message, order_by=['equipment_ID'],
...
def view(item_id):
    ...
    file_path = Path(current_app.static_folder) / 'resource_manifests' / f"{custody_id}_{todaysdate}_InitialSealAudit.csv"
...
def generate_manifest_raw(item_id):
    ...
    file_path = folder_path / f"{custody_id}_{todaysdate}_InitialSealAudit.csv"
