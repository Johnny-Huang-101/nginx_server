...
from lims.models import SpecimenTypes, Assays, SpecimenCollectionContainers
from lims.specimen_types.forms import Add, Edit, Approve, Update
from lims.specimen_types.functions import get_form_choices
from lims.forms import Attach, Import
from lims.view_templates.views import *

item_type = 'Specimen Type'
item_name = 'Specimen Types'
table = SpecimenTypes
table_name = "specimen_types"
...
def view_list():
    ...
    length = SpecimenTypes.query.count() #set to show all specimen_types

    _view_list = view_items(table, item_name, item_type, table_name, length=length, **kwargs)
...
def view(item_id):
    ...
    # item.collection_container = SpecimenCollectionContainers.query.get(2)
    # item.state_id = 2
    # db.session.commit()
