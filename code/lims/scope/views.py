...
from lims.scope.forms import Add, Edit, Approve, Update
from lims.scope.functions import get_form_choices


# Set item global variables
item_type = 'Scope Component(s)'
item_name = 'Scope'
table = Scope
table_name = 'scope'
name = 'component_id'  # This selects what property is displayed in the flash messages
...
def add():
    ...
    if request.method == 'POST':
        if form.validate_on_submit():
            # For each component and assay selected, add the scope item and redirect to the list view
            for component_id in form.component_id.data:
                for assay_id in form.assay_id.data:
                    kwargs['component_id'] = component_id
                    component_name = Components.query.get(component_id).name
                    kwargs['assay_id'] = assay_id
                    assay_name = Assays.query.get(assay_id).assay_name
                    flash_message = (Markup(f'<b>{component_name}</b> was successfully added to <b>{assay_name}</b> scope'), 'success')
                    add_item(form, table, item_type, item_name, table_name, requires_approval, name,
                             custom_flash_message=flash_message, **kwargs)

            return redirect(url_for('scope.view_list'))
...
def update(item_id):
    kwargs = default_kwargs.copy()
    # We don't want the user to update the assay or component, they would need to create
    # a new component and delete this one
    kwargs['disable_fields'] = ['component_id', 'assay_id']
    # Since assay_id is a SelectMultipleField (which takes a list), and assay_id is
    # stored as an int, we need to set the value of the assay_id manually by making
    # it a list
    item = table.query.get(item_id)
    kwargs['assay_id'] = [item.assay_id]
    kwargs['component_id'] = [item.component_id]
...
def view(item_id):
    ...
    alias = f"{item.component.name} | {item.assay.assay_name}"
