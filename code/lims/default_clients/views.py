from lims.models import DefaultClients, Divisions
from lims.forms import Attach, Import
from lims.view_templates.views import *

from lims.default_clients.forms import Add, Edit, Approve, Update
from lims.default_clients.functions import get_form_choices

# Set item global variables
item_type = 'Default Client'
item_name = 'Default Clients'
table = DefaultClients
table_name = 'default_clients'
...
def update(item_id):
    ...
    form = get_form_choices(Update(), item.agency_id)
...
def view(item_id):
    ...
    alias = f"{item.type.name} | {item.agency.name} | {item.division.name}"
...
def get_divisions():
    ...
    divisions = Divisions.query.filter_by(agency_id=agency_id, db_status='Active')
    choices = []
