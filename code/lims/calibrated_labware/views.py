...
from lims.models import CalibratedLabware, Locations, CalibratedLabwareTypes, Statuses
from lims.calibrated_labware.forms import Add, Edit, Approve, Update
from lims.calibrated_labware.functions import get_form_choices
from lims.forms import Attach, Import
from lims.view_templates.views import *

# Set item global variables
item_type = 'Calibrated Labware'
item_name = 'Calibrated Labware'
table = CalibratedLabware
table_name = 'calibrated_labware'
...
def view_list():
    ...
    kwargs['types'] = [item.name for item in CalibratedLabwareTypes.query]
    ...
    # Filter based on calibration labware type
    if query_type == 'type':
        if query:
            items = table.query.join(CalibratedLabwareTypes).filter(CalibratedLabwareTypes.name == query)
    ...
    _view_list = view_items(table, item_name, item_type, table_name,
                            items=items, length=500, filter_message=filter_message, order_by=['equipment_ID'],
                            warning_alerts=warning_alerts,
                            danger_alerts=danger_alerts, **kwargs)
