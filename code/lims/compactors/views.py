...
from lims.models import Compactors, Locations, StorageTypes, Statuses, Specimens, Cases, Containers, SpecimenTypes
from lims.fume_hoods.forms import Add, Edit, Approve, Update
from lims.compactors.forms import Add, Edit, Approve, Update
from lims.compactors.functions import get_form_choices
...
item_type = 'Storage'  # singular
item_name = 'Storage'
table = Compactors
table_name = 'compactors'
...
def view_list():
    ...
    kwargs['types'] = [item.name for item in StorageTypes.query]
    ...
    if query_type == 'type':
        if query:
            items = table.query.join(StorageTypes).filter(StorageTypes.name == query)
    ...
    _view_list = view_items(table, item_name, item_type, table_name, **kwargs,
                            items=items, length=500, order_by=['equipment_id'])
