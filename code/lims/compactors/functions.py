from lims import db
from lims.models import Statuses, StorageTypes
...
def get_form_choices(form):
    ...
    form.storage_type.choices = [(item.id, item.name) for item in StorageTypes.query.all()]
    form.storage_type.choices.insert(0, (0, 'Please select a type'))
