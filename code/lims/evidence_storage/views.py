...
from lims.models import EvidenceStorage, Locations, EvidenceStorageTypes, Statuses, Cases, Specimens, Containers, SpecimenTypes, AttachmentTypes
from lims.evidence_storage.forms import Add, Edit, Approve, Update
from lims.evidence_storage.functions import get_form_choices
...
item_type = 'Evidence Storage'  # singular
item_name = 'Evidence Storage'
table = EvidenceStorage
table_name = 'evidence_storage'
...
def view_list():
    kwargs = {'locations': Locations.query.all(), 'models': models_iter, 'location_dict': location_dict,
              'types': [item.name for item in EvidenceStorageTypes.query],
              'statuses': [item.name for item in Statuses.query]}
    ...
    if query_type == 'type':
        if query:
            items = table.query.join(EvidenceStorageTypes).filter(EvidenceStorageTypes.name == query)
